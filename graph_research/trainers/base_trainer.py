...
class BaseTrainer:
    ...
    def __init__(self, cfg, writer, logger):
        ...
        # if os.path.isfile(self.resume_name):
        #     model_chk = torch.load(self.resume_name)
        #     self.model.load_state_dict(model_chk["model_state"])
        #     print(f'Loaded [resume-file] : {self.resume_name}')
    ...
    def train(self):
            ...
            for phase in ['train', 'val']:
                if phase == 'train':
                    ...
                    if self.monitor == 'loss':
                        if monitor_val < self.best_loss:
                            print(' |--(saved model)-- best {}: {:.6f}\n'.format(self.monitor,monitor_val))
                            self.best_loss = monitor_val
        ...
        if self.monitor == 'loss':
            print(f'Best {self.monitor} | {self.best_loss}')
        else:
            print(f'Best {self.monitor} | {self.best_acc}')
